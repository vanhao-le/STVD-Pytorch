...
from stvd_create_dataset import STVD_Retrain_Dataset, Triplet_STVD_Dataset

import chip.config as config
import matplotlib.pyplot as plt
from chip.siamese_train_model import train_model
from model.HAN import TripletNet
...
def main():    
    ...
    siamese_train_dataset = Triplet_STVD_Dataset(train_dataset)
    siamese_test_dataset = Triplet_STVD_Dataset(val_dataset)
    ...
    embedding_net = EmbeddingNet(pooling='gem')
    ...
    df.to_csv(r'training_data\triplet_log.csv',columns=['epoch','training_loss','val_loss'], header=True, index=False, encoding='utf-8')

    # fit(triplet_train_loader, triplet_test_loader, model, criterion, optimizer, exp_lr_scheduler, num_epochs, device, 1)


    model_save_filename = r'model_assets\triplet_model.pth'
